...
- Feishu/Reaction notifications: add `channels.feishu.reactionNotifications` (`off | own | all`, default `own`) and gate reaction-event dispatch to bot-authored messages by default, preventing default-on reaction ingress from triggering unsolicited group turns; includes reaction notification mode regression coverage. (#28529) thanks @cowboy129.
